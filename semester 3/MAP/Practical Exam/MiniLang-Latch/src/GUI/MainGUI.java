...
public class MainGUI {
    ...
    public MainGUI(Controller controller) {
        ...
        TableColumn<Map.Entry<Integer, Integer>, Integer> latchAddressColumn = new TableColumn<>("Location");
        latchAddressColumn.setCellValueFactory(cd -> new SimpleObjectProperty<>(cd.getValue().getKey()));
        TableColumn<Map.Entry<Integer, Integer>, Integer> latchValueColumn = new TableColumn<>("Value");
        latchValueColumn.setCellValueFactory(cd -> new SimpleObjectProperty<>(cd.getValue().getValue()));
        this.latchTableView.getColumns().setAll(latchAddressColumn, latchValueColumn);
    ...
    public void show() {
        ...
        VBox latchBox = new VBox(6,new Label("Latch Table:"), latchTableView);
        ...
        latchTableView.setPrefHeight(200);
    ...
    public void reload(){
        ...
        if (!programs.isEmpty()) {
            ...
            //latch table
            MyLatchTableI latchTable = programs.get(0).getLatchTable();
            var latchRows = latchTable.getContent().entrySet().stream()
                    .map(e -> new AbstractMap.SimpleEntry<>(e.getKey(), e.getValue()))
                    .toList();
            latchTableView.getItems().setAll(latchRows);
