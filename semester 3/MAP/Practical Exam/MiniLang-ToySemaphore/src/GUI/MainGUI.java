...
public class MainGUI {
    ...
    private TableView<Map.Entry<Integer, Pair<Integer, Pair<List<Integer>, Integer>>>> semaphoreTableView;
    ...
    public MainGUI(Controller controller) {
        ...
        TableColumn<Map.Entry<Integer, Pair<Integer, Pair<List<Integer>, Integer>>>, Integer> semAddressColumn = new TableColumn<>("Address");
        semAddressColumn.setCellValueFactory(cd -> new SimpleObjectProperty<>(cd.getValue().getKey()));
        TableColumn<Map.Entry<Integer, Pair<Integer, Pair<List<Integer>, Integer>>>, Integer> semN1Column = new TableColumn<>("N1");
        semN1Column.setCellValueFactory(cd -> new SimpleObjectProperty<>(cd.getValue().getValue().getKey()));
        TableColumn<Map.Entry<Integer, Pair<Integer, Pair<List<Integer>, Integer>>>, List<Integer>> semListColumn = new TableColumn<>("List");
        semListColumn.setCellValueFactory(cd -> new SimpleObjectProperty<>(cd.getValue().getValue().getValue().getKey()));
        TableColumn<Map.Entry<Integer, Pair<Integer, Pair<List<Integer>, Integer>>>, Integer> semN2Column = new TableColumn<>("N2");
        semN2Column.setCellValueFactory(cd -> new SimpleObjectProperty<>(cd.getValue().getValue().getValue().getValue()));
        this.semaphoreTableView.getColumns().setAll(semAddressColumn, semN1Column, semListColumn, semN2Column);
    ...
    public void show() {
        ...
        VBox semBox  = new VBox(6,new Label("Semaphore Table:"), semaphoreTableView);
    ...
    public void reload(){
        ...
        if (!programs.isEmpty()) {
            ...
            var semaphoreRows = semaphoreTable.getContent().entrySet().stream()
                    .map(e -> new AbstractMap.SimpleEntry<>(e.getKey(), e.getValue()))
                    .toList();
            semaphoreTableView.getItems().setAll(semaphoreRows);
