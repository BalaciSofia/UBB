...
public class NewStatement implements Statement{
    ...
    public ProgramState execute(ProgramState state) throws ModelException, DictException {
        if(!state. getSymTableStack().peek().containsKey(this.label)){
            throw new VarNotDefined("Variable "+this.label+" not defined");
        }
        if(!(state. getSymTableStack().peek().get(this.label).getType() instanceof RefType)){
            throw new VarNotDefined("Variable "+this.label+" is not a reference");
        }

        Value result=this.exp.evaluate(state. getSymTableStack().peek(),state.getHeap());
        RefType refType=(RefType) state. getSymTableStack().peek().get(this.label).getType();
        ...
        state. getSymTableStack().peek().set(this.label,new RefValue(addr,refType.getInner()));
        return null;
    }
    @Override
