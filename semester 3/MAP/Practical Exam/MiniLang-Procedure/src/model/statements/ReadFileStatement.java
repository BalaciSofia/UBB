...
public class ReadFileStatement implements Statement{
    ...
    public ProgramState execute(ProgramState state)throws ModelException, DictException {
        if(state. getSymTableStack().peek().containsKey(varName) &&
        state. getSymTableStack().peek().get(varName).getType().equals(new IntType())){
            Value value = exp.evaluate(state. getSymTableStack().peek(),state.getHeap());
            if (value.getType().equals(new StringType())) {
                ...
                if(state.getFileTable().containsKey(file.getValue())){
                    ...
                    try {
                        ...
                        if (line == null) {
                            state. getSymTableStack().peek().set(varName, new IntType().defaultValue());
                        } else {
                            state. getSymTableStack().peek().set(varName, new IntValue(Integer.parseInt(line)));
                        }
                    } catch (Exception e) {
