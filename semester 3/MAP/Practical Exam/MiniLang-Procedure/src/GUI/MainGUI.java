...
public class MainGUI {
    ...
    private TableView<Map.Entry<String, String>> procedureTableView;
    ...
    public MainGUI(Controller controller) {
        ...
        this.procedureTableView = new TableView<>();
        TableColumn<Map.Entry<String, String>, String> procNameColumn = new TableColumn<>("Name");
        procNameColumn.setCellValueFactory(cd -> new SimpleObjectProperty<>(cd.getValue().getKey()));
        TableColumn<Map.Entry<String, String>, String> procBodyColumn = new TableColumn<>("Body");
        procBodyColumn.setCellValueFactory(cd -> new SimpleObjectProperty<>(cd.getValue().getValue()));
        this.procedureTableView.getColumns().setAll(procNameColumn, procBodyColumn);
    ...
    public void show() {
        ...
        VBox procBox = new VBox(6,new Label("Procedure Table:"), procedureTableView);
        ...
        procedureTableView.setPrefHeight(200);
    ...
    public void reload(){
        ...
        if (!programs.isEmpty()) {
            ...
            //procedure table
            ProceduresTable procTable = programs.get(0).getProcedureTable();
            var procRows = procTable.getContent().entrySet().stream()
                    .map(e -> {
                        String name = e.getKey();
                        List<String> params = e.getValue().getKey();
                        Statement body = e.getValue().getValue();

                        String signature = name + "(" + String.join(",", params) + ")";
                        String bodyStr = body.toString();

                        return new AbstractMap.SimpleEntry<>(signature, bodyStr);
                    })
                    .toList();

            procedureTableView.getItems().setAll(procRows);
            procedureTableView.refresh();

        }
    }

    private void showProgramStateDetails(int id) throws StackException {
        List<ProgramState> programs = controller.getRepository().getProgramList();
        ...
        // sym table
        var symRows = selected. getSymTableStack().peek().getDictionary().entrySet().stream()
                .map(e -> new AbstractMap.SimpleEntry<>(e.getKey(), e.getValue()))
                .toList();
