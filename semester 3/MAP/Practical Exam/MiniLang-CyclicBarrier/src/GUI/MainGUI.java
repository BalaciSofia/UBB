...
public class MainGUI {
    ...
    public MainGUI(Controller controller) {
        ...
        this.barrierTableView = new TableView<>();
        TableColumn<Map.Entry<Integer, Pair<Integer, List<Integer>>>, Integer> barrierAddrColumn = new TableColumn<>("Address");
        barrierAddrColumn.setCellValueFactory(cd -> new SimpleObjectProperty<>(cd.getValue().getKey()));
        TableColumn<Map.Entry<Integer, Pair<Integer, List<Integer>>>, Integer> barrierCountColumn = new TableColumn<>("Count");
        barrierCountColumn.setCellValueFactory(cd -> new SimpleObjectProperty<>(cd.getValue().getValue().getKey()));
        TableColumn<Map.Entry<Integer, Pair<Integer, List<Integer>>>, List<Integer>> barrierListColumn = new TableColumn<>("List");
        barrierListColumn.setCellValueFactory(cd -> new SimpleObjectProperty<>(cd.getValue().getValue().getValue()));
        this.barrierTableView.getColumns().setAll(barrierAddrColumn, barrierCountColumn, barrierListColumn);
    ...
    public void show() {
        ...
        VBox barrierBox = new VBox(6,new Label("Barrier Table:"), barrierTableView);
        ...
        barrierTableView.setPrefHeight(200);
    ...
    public void reload(){
        ...
        if (!programs.isEmpty()) {
            ...
            //barrier table
            MyBarrierI barrierTable = programs.get(0).getBarrierTable();
            var barrierRows = barrierTable.getContent().entrySet().stream()
                    .map(e -> new AbstractMap.SimpleEntry<>(e.getKey(), e.getValue()))
                    .toList();
            barrierTableView.getItems().setAll(barrierRows);
