...
public class MainGUI {
    ...
    public MainGUI(Controller controller) {
        ...
        TableColumn<Map.Entry<Integer, Integer>, Integer> lockIndexColumn = new TableColumn<>("Index");
        lockIndexColumn.setCellValueFactory(cd -> new SimpleObjectProperty<>(cd.getValue().getKey()));
        TableColumn<Map.Entry<Integer, Integer>, Integer> lockValueColumn = new TableColumn<>("Value");
        lockValueColumn.setCellValueFactory(cd -> new SimpleObjectProperty<>(cd.getValue().getValue()));
        this.lockTableView.getColumns().setAll(lockIndexColumn, lockValueColumn);
    ...
    public void show() {
        ...
        VBox lockBox = new VBox(6,new Label("Lock Table:"), lockTableView);
        ...
        lockTableView.setPrefHeight(200);
    ...
    public void reload(){
        ...
        if (!programs.isEmpty()) {
            ...
            //lock table
            MyLockI lockTable = programs.get(0).getLockTable();
            var lockRows = lockTable.getContent().entrySet().stream()
                    .map(e -> new AbstractMap.SimpleEntry<>(e.getKey(), e.getValue()))
                    .toList();
            lockTableView.getItems().setAll(lockRows);
